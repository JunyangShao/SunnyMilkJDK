  ...
  if (lt->is_constant() && rt->is_constant()) {
    ...
    if (l->as_CompareOp() != NULL) {
      ...
      if (lss_sux == eql_sux && eql_sux == gtr_sux) {
        ...
        // canon->set_smf_bcp(x->smf_bcp());
        // canon->set_smf_method(x->smf_method());
        canon->set_smf_8bit_counter_idx(x->smf_8bit_counter_idx());
        canon->set_smf_probe_status(x->smf_probe_status());
        if (cmp->x() == cmp->y()) {
          do_If(canon);
        } else {
