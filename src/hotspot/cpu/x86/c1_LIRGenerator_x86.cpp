...
void LIRGenerator::do_If(If* x) {
  ...
  if (x->x()->type()->is_float_kind()) {
    __ branch(lir_cond(cond), x->tsux(), x->usux(), x->smf_probe_status(), x->smf_8bit_counter_idx());
  } else {
    __ branch(lir_cond(cond), x->tsux(), x->smf_probe_status(), x->smf_8bit_counter_idx());
  }
